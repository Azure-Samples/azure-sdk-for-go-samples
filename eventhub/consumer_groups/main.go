...
func main() {
	...
	consumerGroup, err := createConsumerGroup(ctx, conn)
	if err != nil {
		log.Fatal(err)
	}
...
func createConsumerGroup(ctx context.Context, conn *arm.Connection) (*armeventhub.ConsumerGroup, error) {
	consumerGroupsClient := armeventhub.NewConsumerGroupsClient(conn, subscriptionID)
