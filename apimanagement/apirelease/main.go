...
func main() {
	...
	apiRelease, err := createApiRelease(ctx, conn, *api.ID)
	if err != nil {
		log.Fatal(err)
	}
...
func createApiRelease(ctx context.Context, conn *arm.Connection, apiId string) (*armapimanagement.APIReleaseContract, error) {
	apiReleaseClient := armapimanagement.NewAPIReleaseClient(conn, subscriptionID)
