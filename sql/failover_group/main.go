...
func main() {
	...
	failoverGroup, err := createFailoverGroup(ctx, conn, *partnerServer.ID)
	if err != nil {
		log.Fatal(err)
	}
	log.Println("failover group:", *failoverGroup.ID)

	failoverGroup, err = getFailoverGroup(ctx, conn, *partnerServer.ID)
	if err != nil {
		log.Fatal(err)
	}
...
func createFailoverGroup(ctx context.Context, conn *arm.Connection, partnerServerID string) (*armsql.FailoverGroup, error) {
	failoverGroupsClient := armsql.NewFailoverGroupsClient(conn, subscriptionID)
...
func getFailoverGroup(ctx context.Context, conn *arm.Connection, partnerServerID string) (*armsql.FailoverGroup, error) {
	failoverGroupsClient := armsql.NewFailoverGroupsClient(conn, subscriptionID)
