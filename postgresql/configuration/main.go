...
func main() {
	...
	configuration, err := createConfiguration(ctx, conn)
	if err != nil {
		log.Fatal(err)
	}
...
func createConfiguration(ctx context.Context, conn *arm.Connection) (*armpostgresql.Configuration, error) {
	configurationsClient := armpostgresql.NewConfigurationsClient(conn, subscriptionID)
