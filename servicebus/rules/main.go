...
func main() {
	...
	rules, err := createRules(ctx, conn)
	if err != nil {
		log.Fatal(err)
	}
...
func createRules(ctx context.Context, conn *arm.Connection) (*armservicebus.Rule, error) {
	rulesClient := armservicebus.NewRulesClient(conn, subscriptionID)
