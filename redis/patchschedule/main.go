...
func main() {
	...
	patch, err := createPatchSchedule(ctx, conn)
	if err != nil {
		log.Fatal(err)
	}
...
func createPatchSchedule(ctx context.Context, conn *arm.Connection) (p armredis.PatchSchedulesCreateOrUpdateResponse, err error) {
	patchSchedulesClient := armredis.NewPatchSchedulesClient(conn, subscriptionID)
