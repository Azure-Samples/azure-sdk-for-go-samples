...
func main() {
	...
	gremlinDatabase, err := createGremlinDatabase(ctx, conn)
	if err != nil {
		log.Fatal(err)
	}
	log.Println("cosmos gremlin database:", *gremlinDatabase.ID)

	gremlinGraph, err := createGremlinGraph(ctx, conn)
	if err != nil {
		log.Fatal(err)
	}
...
func createGremlinDatabase(ctx context.Context, conn *arm.Connection) (*armcosmos.GremlinDatabaseGetResults, error) {
	gremlinResourcesClient := armcosmos.NewGremlinResourcesClient(conn, subscriptionID)
...
func createGremlinGraph(ctx context.Context, conn *arm.Connection) (*armcosmos.GremlinGraphGetResults, error) {
	gremlinResourcesClient := armcosmos.NewGremlinResourcesClient(conn, subscriptionID)
